   ...
   <div class="good-page__intro-preview-slider">
      ...
      <div class="good-page__intro-preview-slider-miniatures">
         <img
            ...
            :key="item.product_id"
            :src="`https://stankobox.runova.tech/api/products/image/${id}/${item.name}`"
      ...
      <img
         ...
         v-if="activeIndex !== null && miniatures.length > 0"
         :src="`https://stankobox.runova.tech/api/products/image/${id}/${miniatures[activeIndex].name}`"
         ref="previewImage"
   ...
   async created() {
      ...
      try {
         const response = await axios.get(
            `https://stankobox.runova.tech/api/products/images/${this.id}`
         );
   ...
   watch: {
      activeIndex(newIndex) {
         if (newIndex !== null && this.miniatures.length > 0) {
            this.$nextTick(() => {
               this.$refs.previewImage.src = `https://stankobox.runova.tech/api/products/image/${this.id}/${this.miniatures[newIndex].name}`;
            });
         }
      },
   },
   ...
   computed: {
      imageSources() {
         return this.miniatures.map(
            (item) =>
               `https://stankobox.runova.tech/api/products/image/${this.id}/${item.name}`
         );
      },
   },
};
